...
output_dir = os.path.join("Models", f"ENNC_TrainOnly_{timestamp}")
...
def _normalize_voltage(v, vmin, vmax):
    denom = (vmax - vmin)
    if denom == 0:
        raise ValueError("Voltage normalization denom is 0 (vmax == vmin).")
    return (v - vmin) / denom
...
for f in trainFiles:
    ...
    # Make time continuous across concatenated files (NumPy-safe scalar extraction)
    t0 = float(np.asarray(t[0]).squeeze())
    t_adj = (t - t0) + t_offset
    t_offset = float(np.asarray(t_adj[-1]).squeeze()) + float(base_Ts)
...
# -------------------------- INFO --------------------------
print("=" * 80)
print("ENNC ARCHITECTURE ANALYSIS AND FIXES (TRAIN-ONLY NORMALIZATION)")
print("=" * 80)
...
# -------------------------- PREPROCESSING (TRAIN-ONLY normalization) --------------------------
print(f"\n{'=' * 80}")
print("ENHANCED DATA PREPROCESSING (TRAIN-ONLY NORMALIZATION)")
print(f"{'=' * 80}")
...
# Step 3: Improved normalization (TRAIN-ONLY; no test leakage)
print("Step 3: Improved normalization (TRAIN-ONLY)...")

train_v_min = float(np.min(tr_Vout))
train_v_max = float(np.max(tr_Vout))
train_i_abs_max = float(np.max(np.abs(tr_Iin)))

voltage_range = train_v_max - train_v_min
current_max = train_i_abs_max

headroom = 0.2
minInputVTr = train_v_min
maxInputVTr = train_v_max
...
print("Train-only normalization parameters:")
print(f"  Voltage: [{minInputVTr:.6f}, {maxInputVTr:.6f}] (from TRAIN only)")
print(f"  Current: ±{maxInputITr:.6f} (from TRAIN only)")
...
print("\nOut-of-range report (using TRAIN normalization):")
for t in tests:
    vnorm = t["ts_Vout_norm"]
    inorm = t["ts_Iin_norm"]
    v_oor = float(np.mean((vnorm < 0.0) | (vnorm > 1.0))) * 100.0
    i_oor = float(np.mean((inorm < -1.0) | (inorm > 1.0))) * 100.0
    print(f"  {os.path.basename(t['path'])}: V_out_of_[0,1]={v_oor:.2f}%  I_out_of_[-1,1]={i_oor:.2f}%")
...
print(f"  Temp input:  {'enabled' if Temp_in else 'disabled'}")
...
with open(log_path, "w", encoding="utf-8") as log:
    log.write("ENNC Model Training Log (TRAIN-ONLY NORMALIZATION)\n")
...
plt.title("Training Loss - Fixed ENNC Architecture (Train-only norm)")
...
for t, m in zip(tests, test_metrics):
    ...
    plt.title(f"Predicted vs Actual (Test) - {m['file']} (Train-only norm)")
