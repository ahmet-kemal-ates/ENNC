...
if pdist2(obj.pBest', obj.gBest') < obj.stopThreshold && obj.guaranteedFlag
    obj.velocity = obj.gBest - obj.position + ...
        obj.w*obj.velocity + obj.guaranteedDelta.*(1-2*rand(obj.numParameters, 1));
    % Standard PSO
else
    if obj.fullyInformedFlag
        numNeighbors = size(obj.neighborhood, 2);
        % Get the random coefficient
        r_p = rand(obj.numParameters,numNeighbors);
        r_g = rand(obj.numParameters,1);
        obj.velocity = obj.w*obj.velocity + ...
            obj.c_g.*r_g.*(obj.gBest - obj.position) + ...
            sum(obj.c_p.*r_p.*(obj.neighborhood - obj.position), 2)/numNeighbors;
    else
        % Get the random coefficient
...
% Saturate the particle velocity
