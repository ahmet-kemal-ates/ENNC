...
switch mode
    case 'zero'
        obj.chromosome = zeros(obj.numParameters, 1);
        
    case 'uniform'
        obj.chromosome = range(:,1) + abs(range(:,2)-range(:,1)).*rand(obj.numParameters, 1);
        
    case 'normal'
        avg = mean(range, 2).*ones(obj.numParameters, 1);
        sigma = abs(range(:,2)-range(:,1))/4.*ones(obj.numParameters, 1);
        obj.chromosome = avg+sigma.*randn(obj.numParameters, 1);
        
        above_2sigma_index = obj.chromosome>avg + 2*sigma;
        below_2sigma_index = obj.chromosome<avg - 2*sigma;
        obj.chromosome(above_2sigma_index)=avg(above_2sigma_index)+2*sigma(above_2sigma_index);
        obj.chromosome(below_2sigma_index)=avg(below_2sigma_index)-2*sigma(below_2sigma_index);
end

% Penalty on violation of boundaries
obj.penalty = sum(-(obj.chromosome(obj.chromosome<obj.lb) - obj.lb(obj.chromosome<obj.lb))) + ...
    sum((obj.chromosome(obj.chromosome>obj.Ub) - obj.Ub(obj.chromosome>obj.Ub)));

% Evaluate Fitness
obj.fitness = p_Fitness(obj.chromosome, p_fitnessOptions) + obj.penalty;
