classdef GA
    ...
    methods
        % Constructor
        function obj = GA(p_Fitness, p_numParameters, varargin)
            ...
            default_GAOptions = struct(  ...
                'numElites',     1,                           ... rate of Elite
                'kRate',         0.3,                           ... kRate for K tournament used in GA hybridization
                'mutationRate',  0.4,                           ... Percentage of numIndividuals defining the number of mutation    
                ...
                'crossoverRate', 0.6,                           ... Percentage of numIndividuals defining the number of crossovers  
            ...
            if ~isfield(obj.GAOptions, 'numElites')
                obj.GAOptions.numElites = default_GAOptions.numElites;
            end
            ...
            % Get values
            obj.k = max(2,round(obj.GAOptions.kRate*obj.generalOptions.numIndividuals));
            obj.numCrossover = min(floor(obj.generalOptions.numIndividuals), round(obj.GAOptions.crossoverRate*obj.generalOptions.numIndividuals/2));
            obj.numMutation = min(floor(obj.generalOptions.numIndividuals), round(obj.GAOptions.mutationRate*obj.generalOptions.numIndividuals));

            % Define gBest and old_gBest
            obj.elite = struct('chromosome', [], 'fitness', inf);
            obj.old_elite = struct('chromosome', [], 'fitness', inf);
        end
    end
end